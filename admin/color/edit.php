...
$id = $_GET['id'];
$color = $pdo->query("SELECT * FROM Color WHERE id_color = $id")->fetch(PDO::FETCH_ASSOC);
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $stmt = $pdo->prepare("UPDATE Color SET name = ? WHERE id_color = ?");
...
<div class="container">
    ...
    <div class="admin-info">
        ...
        <div class="admin-dashboard">
            <h2 class="dashboard-title">Редактировать цвет</h2>
            <form method="post">
                <input type="text" name="name" value="<?= htmlspecialchars($color['name']) ?>" required><br>
